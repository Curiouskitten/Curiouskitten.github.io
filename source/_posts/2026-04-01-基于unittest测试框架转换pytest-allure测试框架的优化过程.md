...
6. 夹具支持继承，但只被继承的夹具范围要>=继承夹具，即类级别的夹具是不能继承函数范围的夹具，但反之有效，以下以我封装的app自动化的conftest.py的部分内容为例：先定义了一个名为driver的夹具，功能是的建立针对xxapp的appium驱动对象；后定义了一个名为login_driver的夹具，继承夹具driver，功能是在实现一个用户已登录状态的驱动对象。以便于后续在app中进行其它操作时直接调用而不用反复进行登录操作。
